...
export function DocumentsPage() {
  ...
  const initialPatientId   = Number(urlParams.get("patientId")) || undefined;
  ...
  return (
    <section>
      <header className="page-header with-action">
        <div>
          <span className="eyebrow">Acquisizione e OCR</span>
          <h1>Documenti</h1>
          <p>Cerca, elabora con l'OCR e archivia i documenti.</p>
        </div>
      ...
      {filters.patientId && (
        <div className="dm-toolbar">
          <span className="result-count">
            Archivio documentale di{" "}
            <strong>{patientNames[filters.patientId] ?? `paziente #${filters.patientId}`}</strong>
          </span>
          <button
            type="button"
            className="text-link"
            onClick={() => { setFilters({}); setFilterDraft({}); }}
          >
