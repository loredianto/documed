...
interface Props {
  ...
  patients: Patient[];
  clinicians: Clinico[];
  onProcessed: (extraction: OcrExtraction, documentType: DocumentType) => void | Promise<void>;
 ...
 */
export function DocumentModal({ document: doc, mode, patients, clinicians, onProcessed, onFiled, onClose }: Props) {
  ...
  async function handleSave() {
    ...
    setSaveError("");
    try {
      await onProcessed(confirmedExtraction, docType);
      setView(true);
      setJustSaved(true);
    } catch (error) {
      ...
      <div className="dm-scan-body">
        {justSaved && (
          <div className="dm-save-banner" role="status">
            ...
            Versione digitale salvata. Aggiungila alla cartella clinica del paziente.
          </div>
        )}
        {saveError && (
      ...
      <footer className="dm-scan-footer">
        ...
        <div className="dm-scan-footer-actions">
          {editable ? (
            ...
            <FileActions docId={doc.id} compact filed={doc.filedInRecord} onFiled={() => onFiled?.(doc.id)} />
          )}
        </div>
      </footer>
    </div>
