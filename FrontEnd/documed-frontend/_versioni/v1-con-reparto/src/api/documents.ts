...
export const saveDocumentToRecord = (id: string) =>
  MOCK
    ? mockDelay<{ documentId: string; filed: true }>({ documentId: id, filed: true }, 600)
    : requestJson(`/api/documents/${id}/file-in-record`, { method: "POST" });

/**
 * MOCK front-end — "Inoltra a reparto".
 * Nessun endpoint backend esistente: vedi nota di handoff.
 */
export const forwardDocumentToDepartment = (id: string, department: string) =>
  MOCK
    ? mockDelay<{ documentId: string; department: string }>({ documentId: id, department }, 600)
    : requestJson(`/api/documents/${id}/forward`, { method: "POST", body: JSON.stringify({ department }) });
