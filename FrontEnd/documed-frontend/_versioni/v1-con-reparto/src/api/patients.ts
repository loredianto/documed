...
/**
 * Assegna/trasferisce il ricovero a un reparto di degenza. L'accettazione del
 * reparto destinatario viene notificata dell'arrivo del paziente.
 * Endpoint backend da definire — vedi nota di handoff.
 */
export const assignAdmissionDepartment = (id: number, department: string) => {
  if (MOCK) {
    const now = new Date().toISOString();
    mockAdmissions = mockAdmissions.map((a) =>
      a.id === id ? { ...a, department, updatedAt: now } : a,
    );
    return mockDelay(mockAdmissions.find((a) => a.id === id)!);
  }
  return requestJson<Admission>(`/api/admissions/${id}/department`, {
    method: "POST",
