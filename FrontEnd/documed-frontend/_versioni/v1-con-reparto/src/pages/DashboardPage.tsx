...
import { lastDays, Range, rangeBounds } from "../utils/period";
...
export function DashboardPage() {
  ...
  // Il periodo selezionato governa tutta la pagina, code di lavoro incluse.
  const { fromMs, toMs } = useMemo(() => rangeBounds(range), [range]);
  const inRange = useCallback(
    (iso: string) => { const t = new Date(iso).getTime(); return t >= fromMs && t <= toMs; },
    [fromMs, toMs],
  );

  const incomplete = useMemo(
    () => incompleteAdmissions(admissions, documents).filter((a) => inRange(a.admissionDate)),
    [admissions, documents, inRange],
  );
  const pendingOcr = useMemo(
    () => documents.filter((d) => (d.ocrStatus === "PENDING" || d.ocrStatus === "PROCESSING") && inRange(d.uploadedAt)),
    [documents, inRange],
  );
  const failedOcr = useMemo(
    () => documents.filter((d) => d.ocrStatus === "FAILED" && inRange(d.uploadedAt)),
    [documents, inRange],
  );
  const incompleteAn = useMemo(() => {
    const handled = new Set(admissions.filter((a) => inRange(a.admissionDate)).map((a) => a.patientId));
    return incompletePatients(patientList).filter((p) => handled.has(p.id));
  }, [patientList, admissions, inRange]);
    ...
    <section>
      <header className="page-header with-action">
        <div>
          ...
          <h1>Dashboard</h1>
          <p>Il quadro dell'accettazione: movimenti del periodo e lavoro ancora da completare.</p>
        </div>
            ...
            <Card border={true}>
              <CardBody>
                <div className="d-flex justify-content-between align-items-start">
                  <div>
                    <span className="eyebrow">Movimenti · {activity.length} giorni</span>
            ...
            <Card border={true} className="mt-3">
              <CardBody>
                <div className="d-flex justify-content-between align-items-center mb-1">
                  <div>
                    <span className="eyebrow">Da fare ora</span>
                    <h2 className="panel-title">Lavoro in sospeso</h2>
                  </div>
                      ...
                      <li key={t.key}>
                        <Link to={t.to} className="dm-worklist-row" data-tone={empty ? "done" : t.tone}>
                          ...
                          <span className="dm-worklist-text">
                            ...
                            <span className="dm-worklist-hint">{empty ? "nessun arretrato nel periodo" : t.hint}</span>
                          </span>
